...
const handleBarcodeChange = (barcode: string) => {
    router.get('dashboard', { barcode: barcode }, {
...
const getTotalSales = () => {
    router.get('dashboard', { EOTD: true }, {
        preserveScroll: true, preserveState: true,
        onError: (errors) => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col  rounded-xl">
            <div class="flex gap-1">
                <!-- Left Panel (25%) -->
                <Left :selected-items="selectedItems" :total-order-price="totalOrderPrice"
                    :totalOrderPriceCounterCurrency="totalOrderPriceCounterCurrency" @payCart="payCart"
                    @removeItemFromOrder="removeItemFromOrder" @handleBarcodeChange="handleBarcodeChange"
                    @addItemToOrder="addItemToOrder" />
                <!-- Middle Panel (50%) -->
                <Center :categories="props.categories" :selectedCategory="selectedCategory" :mergedItems="mergedItems"
                    @add-item-to-order="addItemToOrder" @load-items="loadItems"
